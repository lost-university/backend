...
class TestCreatePlan:
    def test_create_plan(self, test_client: TestClient) -> None:
        ...
        response = test_client.post("/api/plans", json=request_data)
...
class TestDeletePlan:
    def test_delete_plan(self, test_client: TestClient) -> None:
        ...
        response = test_client.delete(f"/api/plans/{plan_id}")
    ...
    def test_delete_nonexisting_plan(self, test_client: TestClient) -> None:
        ...
        response = test_client.delete(f"/api/plans/{uuid.uuid4()}")
        assert response.status_code == 404


class TestBadDB:
    def test_get_plans(self, test_client: TestClient, overwrite_session_dependency: Iterator[None]) -> None:
        response = test_client.get("/api/plans")
    ...
    def test_delete_plan(self, test_client: TestClient, overwrite_session_dependency: Iterator[None]) -> None:
        response = test_client.delete(f"/api/plans/{uuid.uuid4()}")
        assert response.status_code == 500
